...
public class LibraryController {
    ...
    @GetMapping
    public ResponseEntity<?> all(@AuthenticationPrincipal AuthenticatedUser user,
    ...
    @PostMapping
    public ResponseEntity<SongResponse> create(@AuthenticationPrincipal AuthenticatedUser user,
                                               @Valid @RequestBody SongRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.create(user.id(), request));
    }

    @PutMapping("/{id}")
    public SongResponse update(@AuthenticationPrincipal AuthenticatedUser user,
